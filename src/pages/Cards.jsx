    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent sx={{ background: colors.bgSecondary, pt: 2 }}>
        <Box sx={{ mb: 2 }}>
          ...
          >
            Card ID:{" "}
            <strong style={{ color: "#818cf8" }}>{card?.card_id}</strong>
          </Typography>
...
export default function Cards() {
  ...
  const handleRevoke = (cardId) => {
    setConfirmDialog({
      ...
      title: "Thu hồi thẻ",
      message:
        "Bạn có chắc muốn thu hồi thẻ này? Người dùng sẽ không thể sử dụng thẻ để vào cửa sau khi thu hồi.",
      onConfirm: async () => {
  ...
  const handleReactivate = (cardId) => {
    setConfirmDialog({
      ...
      message:
        "Bạn có chắc muốn kích hoạt lại thẻ này? Thẻ sẽ hoạt động trở lại và người dùng có thể sử dụng để vào cửa.",
      onConfirm: async () => {
        setActionLoading(true);
        try {
  ...
  const handleDelete = (cardId) => {
    setConfirmDialog({
      ...
      title: "Xóa thẻ",
      message:
        "Bạn có chắc muốn xóa thẻ này? Hành động này không thể hoàn tác và tất cả dữ liệu liên quan sẽ bị mất.",
      onConfirm: async () => {
        setActionLoading(true);
        try {
